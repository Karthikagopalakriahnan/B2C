...
if db:
    ...
    product_weight_unit=row[7]
    product_price_unit=row[8]
    product_image=row[9]
    product_in_stock=row[10]
    deltab=row[11]
    print(row)
    if(int(qty)>int(product_in_stock)):
        ...
        if(cursor.execute(selectQuery1)>0):
            ...
            try:
                insertQuery="insert into tbl_shopcart values(%d,'%s','%s','%s',%d,'%s','%s','%s',%d,'%s','%s')"%(int(product_id),product_name,category_name,subcategory_name,int(product_price),product_price_unit,
                  product_weight_unit,product_image,int(qty),sessionid,LoginName)
